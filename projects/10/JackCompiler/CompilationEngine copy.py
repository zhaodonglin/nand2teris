...
def compile_class_var_dec(tokens, i):
	...
	while token_str in var_scope_table:
		...
		_, _, i = parse_token(tokens, i, expected_token_str=type_table)
...
def compile_subroutine_dec(tokens, i):
	...
	print expected_type
	_, _, i = parse_token(tokens, i, expected_token_str=expected_type)
...
def compile_let(tokens, i):
	...
	_, _, i = parse_token(tokens, i, expected_token_str=var_names)
	token_str, token_type = tokens[i]
	if token_str == '[':
		_,_,i = parse_token(tokens, i)
...
def compile_term(tokens, i):
	...
	if token_type in keywordConstant or token_type in ['integerConstant', 'stringConstant']:
		_, _, i = parse_token(tokens,i)
		xml_str.append('</term>')
		return i
...
def compile_expression_list(tokens, i):
	...
	if token_str == ')':
		...
		token_str, token_type = tokens[i]
		while token_str == ',':
			i = compile_expression(tokens,i, xml_str)
			token_str, token_type = tokens[i]
	xml_str.append('</expressionList>')
	return i
